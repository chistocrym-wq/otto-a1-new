...
export function AccountPage({progress,activity}:Props){
 ...
 return <div className="otto-hub-screen animate-fade-in">
  ...
  <section className="mt-4 rounded-[24px] border border-[#d9e7e2] bg-[#F4F7F2] p-4"><p className="otto-kicker">{t('trainNow')}</p><h2 className="mt-1 text-lg font-black text-slate-950">{weak?`${MODULE_META[weak.id].title} — ${weak.accuracy}%`:t('insufficient')}</h2><p className="mt-2 text-sm leading-6 text-slate-600">{weak?(lang==='de'?'Das ist aktuell der schwächste gemessene Bereich. OTTO wird ihn im Tagesplan häufiger einsetzen.':'Сейчас этому навыку нужно больше внимания. Отто будет чаще включать его в план тренировки.'):(lang==='de'?'Machen Sie zuerst Aufgaben in allen vier Bereichen.':'Сначала выполните задания во всех четырёх модулях.')}</p></section>
