...
function Word({token}:{token:string}){
  ...
  return <span className="relative inline-block align-baseline" onBlur={e=>{if(!e.currentTarget.contains(e.relatedTarget as Node|null))setOpen(false)}}>
    ...
    >{token}</span>
    {open&&translation&&<span className="absolute left-1/2 top-[calc(100%+2px)] z-50 w-max max-w-[220px] -translate-x-1/2 text-center text-xs font-semibold leading-4 text-teal-800">{translation}</span>}
  </span>;
}
