...
export function MyErrorsPage({progress,activity,onSelectModule}:Props){
  ...
  return <div className="animate-fade-in space-y-4 pb-8 text-[var(--otto-ink)]" style={sansFont}>
    <section className="rounded-[28px] border border-[var(--otto-line)] bg-[var(--otto-surface)] p-5 shadow-[0_14px_34px_rgba(43,54,54,.07)] sm:p-6">
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--otto-muted)]">Отто использует только реальные результаты тренировок: ошибки Lesen/Hören, разбор Schreiben и измеренные результаты Sprechen. Если данных мало, слабое место не придумывается.</p>
    </section>

    {!insights.hasData&&<section className="rounded-[24px] border border-[var(--otto-line)] bg-[var(--otto-amber-soft)] p-5"><div className="flex gap-3"><Target className="mt-0.5 h-5 w-5 shrink-0 text-[var(--otto-terracotta-dark)]"/><div><h2 className="font-bold leading-[1.08] text-[var(--otto-ink)]" style={serifFont}>Сначала нужна небольшая диагностика</h2><p className="mt-1 text-sm leading-6 text-[var(--otto-muted)]">Выполните несколько заданий хотя бы в двух модулях. После этого здесь появятся сильная сторона, повторяющиеся проблемы и следующий шаг.</p></div></div></section>}
    ...
    <div className="grid gap-3 sm:grid-cols-2">
      <section className="rounded-[24px] border border-[var(--otto-line)] bg-[var(--otto-sage-soft)] p-5">
        ...
        {strongest?<><h2 className="mt-3 text-xl font-bold leading-[1.08] text-[var(--otto-ink)]" style={serifFont}>{moduleInsightLabel(strongest.id)}</h2><p className="mt-1 text-sm leading-6 text-[var(--otto-muted)]">Средний результат по доступным попыткам: <b>{strongest.average}%</b>. Сейчас это наиболее устойчивый навык по накопленным данным.</p></>:<p className="mt-3 text-sm leading-6 text-[var(--otto-muted)]">Пока недостаточно попыток, чтобы назвать самый сильный навык.</p>}
      </section>
      <section className="rounded-[24px] border border-[var(--otto-line)] bg-[var(--otto-amber-soft)] p-5">
        ...
        {topProblems[0]?<><h2 className="mt-3 text-xl font-bold leading-[1.08] text-[var(--otto-ink)]" style={serifFont}>{topProblems[0].label}</h2><p className="mt-1 text-sm leading-6 text-[var(--otto-muted)]">Эта проблема встречалась <b>{topProblems[0].count}</b> {topProblems[0].count===1?'раз':'раз(а)'}.{topProblems[0].example?` Пример: ${topProblems[0].example}`:''}</p></>:weakest?<><h2 className="mt-3 text-xl font-bold leading-[1.08] text-[var(--otto-ink)]" style={serifFont}>{moduleInsightLabel(weakest.id)}</h2><p className="mt-1 text-sm leading-6 text-[var(--otto-muted)]">Средний результат: <b>{weakest.average}%</b>. Детальные повторяющиеся ошибки появятся после следующих заданий.</p></>:<p className="mt-3 text-sm leading-6 text-[var(--otto-muted)]">Слабое место появится после нескольких тренировок.</p>}
      </section>
    </div>
    ...
    <section className="rounded-[24px] border border-[var(--otto-line)] bg-[var(--otto-surface)] p-5 shadow-[0_12px_30px_rgba(43,54,54,.06)]">
      ...
      <p className="mt-2 text-sm leading-6 text-[var(--otto-muted)]">{nextModule?'Отто направляет в модуль, связанный с самой частой фактической проблемой или самым слабым измеренным навыком. После новых результатов маршрут изменится автоматически.':'Начните с любого раздела — после первых результатов Отто сможет дать персональное направление.'}</p>
