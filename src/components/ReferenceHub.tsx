...
function HorenPage(){return <div className="space-y-3">{HOREN_SECTIONS.map(section=><BulletSection key={section.title} {...section}/>)}<div className="grid gap-3 sm:grid-cols-2"><BulletSection title="Слушать особенно внимательно" points={['числа и цены','телефонные номера','время','даты','дни недели']}/><BulletSection title="Повороты смысла" points={['nicht / kein / keine','aber / leider','doch / sondern','zuerst / dann']}/></div><p className="text-xs leading-5 text-[var(--otto-muted)]">Количество прослушиваний сверено по официальному Goethe-Institut A1 Modellsatz: Teil 1 — дважды, Teil 2 — один раз, Teil 3 — дважды.</p></div>}
...
function SprechenPage(){return <div className="space-y-4"><BulletSection title="TEIL 1 · О себе" points={['Name','Alter','Land','Wohnort','Sprachen','Beruf','Hobby','Потренируйте spelling имени, фамилии, города и страны; повторите номер телефона и индекс.']}/><section className="rounded-2xl border border-[var(--otto-line)] bg-[var(--otto-bg-soft)] p-4"><h2 className="font-bold">TEIL 2 · Задать вопрос</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{QUESTION_BASES.map(item=><SimpleGermanRow key={item.de} de={item.de} ru={item.ru}/>)}</div></section><section className="rounded-2xl border border-[var(--otto-line)] bg-[var(--otto-bg-soft)] p-4"><h2 className="font-bold">TEIL 3 · Просьба</h2><div className="mt-3 grid gap-2 sm:grid-cols-2">{['Bringen Sie mir bitte …','Geben Sie mir bitte …','Öffnen Sie bitte …','Können Sie bitte …?'].map(line=><SimpleGermanRow key={line} de={line} ru="готовая основа просьбы"/>)}</div><h3 className="mt-4 text-sm font-bold">Короткие ответы</h3><div className="mt-2 grid gap-2 sm:grid-cols-3">{['Ja, gerne.','Ja, natürlich.','Ja, kein Problem.'].map(line=><SimpleGermanRow key={line} de={line} ru="короткая естественная реакция"/>)}</div></section></div>}
...
function ReferencePage({page,onBack,onOpen}:{page:ReferencePageId;onBack:()=>void;onOpen:(id:ReferencePageId)=>void}){
  ...
  else if(page==='horen')body=<HorenPage/>;
