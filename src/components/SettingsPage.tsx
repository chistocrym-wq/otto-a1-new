...
export function SettingsPage({onOpenSupport,onShare,profile,learningMode,onLearningModeChange,onGenderChange,onEditProfile,onOpenAccount}:Props){
  ...
  return <div className="otto-hub-screen otto-settings-screen animate-fade-in">
    ...
    <section className="rounded-[24px] border border-white/90 bg-white p-4 shadow-[0_12px_32px_rgba(15,23,42,.08)] sm:p-5">
      ...
      {profile&&<fieldset className="mt-4"><legend className="text-sm font-bold text-slate-700">Ваш пол</legend><div className="mt-2 grid grid-cols-2 gap-2"><button type="button" onClick={()=>onGenderChange('female')} aria-pressed={profile.gender==='female'} className={`min-h-12 rounded-xl border px-4 font-bold ${profile.gender==='female'?'border-[#0F7D74] bg-[#EAF4F0] text-[#285C59]':'border-slate-200 bg-white text-slate-600'}`}>Женский</button><button type="button" onClick={()=>onGenderChange('male')} aria-pressed={profile.gender==='male'} className={`min-h-12 rounded-xl border px-4 font-bold ${profile.gender==='male'?'border-[#0F7D74] bg-[#EAF4F0] text-[#285C59]':'border-slate-200 bg-white text-slate-600'}`}>Мужской</button></div>{!profile.gender&&<p className="mt-2 text-xs leading-5 text-slate-500">Пол не указан — используем нейтральные формулировки.</p>}</fieldset>}
    ...
    <section className="mt-4 rounded-[24px] border border-white/90 bg-white p-4 sm:p-5"><div className="flex gap-3"><span className="otto-setting-icon"><Smartphone/></span><div className="min-w-0 flex-1"><h2 className="font-black text-slate-950">{t('install')}</h2><p className="mt-1 text-sm leading-6 text-slate-600">{installed?(lang==='de'?'OTTO läuft bereits als eigenständige App.':'OTTO уже установлен и запускается как отдельное приложение.'):(lang==='de'?'Installieren Sie OTTO auf diesem Gerät und öffnen Sie es anschließend wie eine normale App.':'Установите OTTO на устройство, чтобы запускать его как обычное приложение.')}</p><button type="button" onClick={()=>void install()} disabled={installing} className="mt-3 min-h-12 w-full rounded-xl border border-slate-200 bg-white px-4 font-bold text-slate-700 disabled:opacity-50">{installing?(lang==='de'?'Installation…':'Установка…'):installed?(lang==='de'?'App ist installiert':'Приложение установлено'):promptReady?(lang==='de'?'App installieren':'Установить приложение'):(lang==='de'?'App installieren':'Установить приложение')}</button>{installNotice&&<p role="status" className="mt-3 rounded-xl bg-[#F4F7F2] px-3 py-2 text-sm leading-6 text-slate-700">{installNotice}</p>}</div></div></section>
