...
export function Dashboard(props:DashboardProps){
 ...
 return <div className="animate-fade-in h-full space-y-4 overflow-y-auto overflow-x-hidden overscroll-contain pb-5 text-[var(--otto-ink)] [scrollbar-width:none]" style={sansFont}>
   ...
   <section className="rounded-[24px] border border-[var(--otto-line)] bg-[var(--otto-surface)] p-4 shadow-[0_12px_30px_rgba(43,54,54,.07)] sm:p-5">
     ...
     <div className="mt-4 grid grid-cols-3 gap-2">{([5,15,30] as Minutes[]).map(value=><button key={value} type="button" onClick={()=>chooseMinutes(value)} aria-pressed={minutes===value} className={`otto-dashboard-time-option min-h-16 rounded-2xl border px-2 transition ${minutes===value?'is-selected border-[var(--otto-petrol)] bg-[var(--otto-petrol-soft)] text-[var(--otto-petrol-dark)]':'border-[var(--otto-line)] bg-[var(--otto-bg-soft)] text-[var(--otto-muted)]'}`}><strong className="block text-xl">{value}</strong><span className="text-xs font-bold">{t('minutes')}</span></button>)}</div>
